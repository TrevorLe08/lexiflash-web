    ...
    <div className="inline-flex items-center gap-1.5">
      <button
        ...
        title={
          disabled
            ? undefined
            : `Phát âm (${accent === "en-GB" ? "Anh - Anh 🇬🇧" : "Anh - Mỹ 🇺🇸"})`
        }
        className={cn(
          "rounded-full bg-[#2e3856]/80 text-[#d9dde8] border border-[#3c476c] transition-all duration-200 shadow-sm",
          !disabled &&
            "hover:bg-[#4257B2] hover:text-white hover:border-transparent active:scale-90 cursor-pointer",
      ...
      {showAccentToggle && (
        <button
          ...
          title={
            disabled
              ? undefined
              : `Bấm để đổi giọng đọc (Hiện tại: ${accent === "en-GB" ? "Anh - Anh (UK)" : "Anh - Mỹ (US)"})`
          }
          className={cn(
            ...
            !disabled &&
              "hover:bg-white/[0.12] hover:text-white cursor-pointer",
            disabled && "opacity-40 pointer-events-none cursor-not-allowed",
          )}
        >
